...
public class JsonToJpdl {
	...
	private JsonToJpdl(JSONObject process) {
		...
		try {
			...
			for(int i = 0; i < processElements.length(); i++) {
				...
				this.children.put(currentElement.getString("resourceId"), currentElement);
			}
	...
	public String transform() throws InvalidModelException {
		...
		try {
			if (!this.processData.getJSONObject("properties").getString("ssextension")
	...
	public static String transformRequieredAttribute(String name, String value) throws InvalidModelException {
		if(value == null) 
			throw new InvalidModelException("Attribute " + name + " is missing.");
