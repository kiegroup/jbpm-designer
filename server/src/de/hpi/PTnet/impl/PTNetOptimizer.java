	...
	public boolean removeRedundantPlaces() {
		...
		for (int i=0; i<places.length; i++) {
			...
			for (int j=i+1; j<places.length; j++) {
				...
				list.clear();
				for (Iterator<FlowRelationship> it=p1.getIncomingFlowRelationships().iterator(); it.hasNext(); )
					list.add((Transition)it.next().getSource());
				for (Iterator<FlowRelationship> it=p2.getIncomingFlowRelationships().iterator(); it.hasNext(); ) {
				...
				if (!sameTransitions) 
					continue;
				list.clear();
				for (Iterator<FlowRelationship> it=p1.getOutgoingFlowRelationships().iterator(); it.hasNext(); )
					list.add((Transition)it.next().getTarget());
				for (Iterator<FlowRelationship> it=p2.getOutgoingFlowRelationships().iterator(); it.hasNext(); ) {
	...
	public boolean removeRedundantTransitions() {
		...
		for (int i=0; i<transitions.length; i++) {
			...
			for (int j=i+1; j<transitions.length; j++) {
				...
				list.clear();
				for (Iterator<FlowRelationship> it=t1.getIncomingFlowRelationships().iterator(); it.hasNext(); )
					list.add((Place)it.next().getSource());
				for (Iterator<FlowRelationship> it=t2.getIncomingFlowRelationships().iterator(); it.hasNext(); ) {
				...
				if (!samePlaces) 
					continue;
				list.clear();
				for (Iterator<FlowRelationship> it=t1.getOutgoingFlowRelationships().iterator(); it.hasNext(); )
					list.add((Place)it.next().getTarget());
				for (Iterator<FlowRelationship> it=t2.getOutgoingFlowRelationships().iterator(); it.hasNext(); ) {
					if (!list.contains((Place)it.next().getTarget())) {
						samePlaces = false;
						break;
					}
				}
