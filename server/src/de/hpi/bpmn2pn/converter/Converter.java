...
public abstract class Converter {
	...
	protected class SubProcessPlaces {
		Place startP, endP;
		Place ok, nok, enabled, cancel;
	}
	...
	protected class ConversionContext {
		...
		public Map<Object,Place> map = new HashMap(); // key = edge or event-based gateway or attached event, value = place
	...
	protected void handleSequenceFlow(PetriNet net, SequenceFlow flow, ConversionContext c) {
	...
	// basic idea: all flows sharing their source or their origin are realized as one place
	...
	protected void addMessageFlowRecursively(PetriNet net, MessageFlow flow, ConversionContext c, List<MessageFlow> allFlows, Place p) {
	...
	protected void handleNodesRecursively(PetriNet net, Container container, ConversionContext c) {
...
//		addFlowRelationship(net, pl.endP, connectorT);
	...
	protected void handleANDGateway(PetriNet net, ANDGateway gateway, ConversionContext c) {
	...
	// assumption: at least one input and at least one output edge
	protected void handleXORDataBasedGateway(PetriNet net, XORDataBasedGateway gateway, ConversionContext c) {
		Place p = null;
		if (gateway.getIncomingEdges().size() > 1 && gateway.getOutgoingEdges().size() > 1) {
		...
		if (gateway.getOutgoingEdges().size() > 1 || gateway.getIncomingEdges().size() == 1) {
			for (Edge e: gateway.getOutgoingEdges()) {
	...
	// assumption: at least one input and at least one output edge
	protected void handleXOREventBasedGateway(PetriNet net, XOREventBasedGateway gateway, ConversionContext c) {
		...
		// question: why do we create a transition even in those cases where there is only one incoming branch?
		// answer: because there was a place created for the incoming sequence flow and for the gateway.
		//     in order to merge these places, the sequence flow mapping would need to be altered
	}

	// assumption: at least one input and at least one output edge
	protected void handleORGateway(PetriNet net, ORGateway gateway, ConversionContext c) {
		// not implemented
	}

	// assumption: at least one input and at least one output edge
	protected void handleComplexGateway(PetriNet net, ComplexGateway gateway, ConversionContext c) {
		// not implemented
	}
	...
	protected void prepareExceptionHandling(PetriNet net, SubProcess process, Transition startT, Transition endT, ConversionContext c) {
		SubProcessPlaces pl = c.getSubprocessPlaces(process);
...
//		addFlowRelationship(net, pl.enabled, startT);
		...
		if (c.ancestorHasExcpH) {
			...
			Transition tcancel = addTauTransition(net, "cancel"+process.getId());
			addFlowRelationship(net, pl.ok, tcancel);
			addFlowRelationship(net, tcancel, pl.nok);
			addFlowRelationship(net, tcancel, pl.cancel);
			SubProcessPlaces parentpl = c.getSubprocessPlaces(process.getProcess());
...
//			addFlowRelationship(net, tnok, pl.enabled);
			addFlowRelationship(net, tnok, c.map.get(getOutgoingSequenceFlow(process)));
		}
	}

	// assumption: exactly one output edge
	protected void handleAttachedIntermediateEventForSubProcess(PetriNet net, IntermediateEvent event, ConversionContext c) {
		Transition t = addLabeledTransition(net, event.getId(), event.getLabel());
		handleMessageFlow(net, event, t, t, c);
		
		SubProcessPlaces pl = c.getSubprocessPlaces((SubProcess)event.getActivity());
...
//		addFlowRelationship(net, t, pl.enabled);
	...
	// assumption: exactly one output edge
	protected void handleAttachedIntermediateEventForTask(PetriNet net, IntermediateEvent event, ConversionContext c) {
		Transition t = addLabeledTransition(net, event.getId(), event.getLabel());
		handleMessageFlow(net, event, t, t, c);
	...
	// precondition: there actually is an enclosing exception
	protected void handleExceptions(PetriNet net, Node node, Transition t, ConversionContext c) {
		// skip transition
	...
	protected ControlFlow getIncomingSequenceFlow(Node node) {
		for (Edge edge: node.getIncomingEdges())
	...
	protected ControlFlow getOutgoingSequenceFlow(Node node) {
		for (Edge edge: node.getOutgoingEdges())
			if (edge instanceof ControlFlow)
				return (ControlFlow)edge;
		return null;
	...
	protected Place addPlace(PetriNet net, String id) {
	...
	protected TauTransition addTauTransition(PetriNet net, String id) {
	...
	protected LabeledTransition addLabeledTransition(PetriNet net, String id, String label) {
	...
	protected FlowRelationship addFlowRelationship(PetriNet net, de.hpi.petrinet.Node source, de.hpi.petrinet.Node target) {
