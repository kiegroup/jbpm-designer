...
//		optimizer.removeRedundantTransitions(newtransitions);
	...
	protected boolean isLoopingTransition(Transition t) {
		if (t.getIncomingFlowRelationships().size() != t.getOutgoingFlowRelationships().size())
			return false;
		for (Iterator<FlowRelationship> it=t.getIncomingFlowRelationships().iterator(); it.hasNext(); ) {
			Place p = (Place)it.next().getSource();
			if (!isOutputPlace(t, p))
				return false;
		}
		for (Iterator<FlowRelationship> it=t.getOutgoingFlowRelationships().iterator(); it.hasNext(); ) {
	...
	protected boolean isInputPlace(Place p, Transition i) {
		for (Iterator<FlowRelationship> it=i.getIncomingFlowRelationships().iterator(); it.hasNext(); ) {
	...
	protected boolean hasOtherInputsOrMarkedInputs(PetriNet net, Transition i2, Transition i) {
		for (Iterator<FlowRelationship> iter=i2.getIncomingFlowRelationships().iterator(); iter.hasNext(); ) {
			Place p = (Place)iter.next().getSource();
			
			if (net.getInitialMarking().getNumTokens(p) > 0)
				return true;
			
			for (Iterator<FlowRelationship> iter2=p.getIncomingFlowRelationships().iterator(); iter2.hasNext(); ) {
	...
	protected boolean sharesInputPlace(Transition i) {
		for (Iterator<FlowRelationship> iter=i.getIncomingFlowRelationships().iterator(); iter.hasNext(); ) {
...
//					if (!isOutputPlace(t, p))
	...
	protected boolean isRedundant(Transition i) {
		...
		if (in.size() > 0)
			for (Iterator<FlowRelationship> it=in.get(0).getOutgoingFlowRelationships().iterator(); it.hasNext(); ) {
				Transition t = (Transition)it.next().getTarget();
				if (t != i && t instanceof TauTransition && hasSameInputAndOutputPlaces(t, in, out))
					return true;
			}
		else if (out.size() > 0)
			for (Iterator<FlowRelationship> it=out.get(0).getIncomingFlowRelationships().iterator(); it.hasNext(); ) {
