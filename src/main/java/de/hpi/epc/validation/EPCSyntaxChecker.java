...
public class EPCSyntaxChecker extends AbstractSyntaxChecker {
     ...
     * "Node must be connected with edges"; private static final String
     * NOT_CONNECTED_2 = "Node must be connected with more edges"; private
     * static final String TOO_MANY_EDGES = "Node has too many connected edges";
     * private static final String NO_CORRECT_CONNECTOR =
     * "Node is no correct connector";
     * 
     * private static final String MANY_STARTS =
     * "There must be only one start event"; //private static final String
     * MANY_ENDS = "There must be only one end event";
     * 
     * private static final String FUNCTION_AFTER_OR =
     * "There must be no functions after a splitting OR/XOR"; private static
     * final String PI_AFTER_OR =
     * "There must be no process interface after a splitting OR/XOR"; private
     * static final String FUNCTION_AFTER_FUNCTION =
     * "There must be no function after a function"; private static final String
     * EVENT_AFTER_EVENT = "There must be no event after an event"; private
     * static final String PI_AFTER_FUNCTION =
     * "There must be no process interface after a function"; private static
     * final String FUNCTION_AFTER_PI =
     * "There must be no function after a process interface";
     */
    ...
    // private static final String MANY_ENDS =
    // "There must be only one end event";
        ...
        for (DiagramNode node : diagram.getNodes()) {
            ...
            if ("Event".equals(node.getType())) {
                ...
                if (in == 1 && out == 0) {
                    ...
                    addError(node, NOT_CONNECTED_2);
                for (DiagramNode next : getNextEventsOrFunctions(node.getOutgoingEdges())) {
                    if (checkFunctionFollowsFunction && "Function".equals(next.getType()))
                        addError(next, FUNCTION_AFTER_FUNCTION);
                    if (checkFunctionFollowsFunction && "ProcessInterface".equals(next.getType()))
                        addError(next, PI_AFTER_FUNCTION);
                }
                ...
                for (DiagramNode next : getNextEventsOrFunctions(node.getOutgoingEdges())) {
                    if (checkFunctionFollowsFunction && "Function".equals(next.getType()))
                        addError(next, FUNCTION_AFTER_PI);
                }
