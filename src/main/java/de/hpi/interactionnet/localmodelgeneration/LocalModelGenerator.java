...
public class LocalModelGenerator {
        ...
        while ((nt = findNextTransition(net, roleName, admin)) != null) {
            ...
            for (Transition t : tlist) {
                if (isLoopingTransition(t)) {
                    applyRule1(net, t);
                } else if (!sharesInputPlace(t)) { // ||
                                                   // !existsAlternativeEnablement(net,
                                                   // t)) {
                    applyRule2(net, t);
                } else {
                    applyRule3(net, t);
    ...
    private NextTransition findNextTransition(InteractionNet newnet, String roleName, StateSpaceAdmin admin) {
        for (Transition t : newnet.getTransitions()) {
            if (inRoles(t, roleName))
                continue;
            if (t instanceof SilentTransition && !isInChoice(t))
                continue;
    ...
    protected void applyRule3(PetriNet net, Transition i) {
        ...
        for (Transition i2 : succ) {
            ...
            if (isBadSequence(i, i2)) {
                ...
                if (hasOtherInputsOrMarkedInputs(net, i2, i)) { // this is an
                                                                // optimization
                                                                // for avoiding
                                                                // unnecessary
                                                                // duplication
    ...
    // therefore, conflict concerning input token or double creation of a token
    // is not possible
    ...
    protected boolean sharesInputPlace(Transition i) {
        for (FlowRelationship rel : i.getIncomingFlowRelationships()) {
            Place p = (Place) rel.getSource();
            // assumption: there are not two flow relationships with the same
            // source / target
    ...
    // checks whether ALL output places could also be marked by other
    // transitions
