...
public class InteractionNetReducer extends PTNetReducer {
    ...
    protected void logIt(PetriNet net, Transition i) {
        try {
            ...
            if (isBounded)
                numUnreachable = net.getTransitions().size()
                        - new StateSpaceCalculator(new PTNetInterpreter(), (PTNet) net, (Marking) net.getInitialMarking()).getReachableTransitions().size();
            saver.saveDocumentToFile(doc, "test/de/hpi/interactionnet/test/testlog/log_" + (fileCounter++) + "_" + i + "_#unreachable=" + numUnreachable
                    + "_bounded=" + isBounded + "_#t=" + net.getTransitions().size() + "_#p=" + net.getPlaces().size() + ".pnml");
        } catch (Exception e) {
        }
    }
