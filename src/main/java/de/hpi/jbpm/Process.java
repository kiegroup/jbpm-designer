    ...
    public void createTransitions() {
        ...
        for (Node node = root.getFirstChild(); node != null; node = node.getNextSibling()) {
            if (!node.getNodeName().equals("#text")) {
                ...
                try {
                    String currentStencilName = node.getAttributes().getNamedItem("name").getNodeValue();
                ...
                if (node.hasChildNodes()) {
                    for (Node item = node.getFirstChild(); item != null; item = item.getNextSibling()) {
                        if (item.getNodeName().equals("transition")) {
                            ...
                            t.setStart(new Docker(currentStencil.getBounds().getWidth() / 2, currentStencil.getBounds().getHeight() / 2));
                            outgoings.add(t);
                        }
                    }
                }
