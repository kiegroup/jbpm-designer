...
public class Transition {
            ...
            for (int i = 0; i < pathDockers.length; i++) {
                if (pathDockers[i].length() > 1) {
                    ...
                    if (dockerPosition.length == 2) {
                        Docker d = new Docker(Integer.parseInt(dockerPosition[0]), Integer.parseInt(dockerPosition[1]));
                        dockers.add(d);
                    }
                }
            }
    ...
    public JSONObject toJson() throws JSONException {
        ...
        end = new Docker(targetNode.getBounds().getWidth() / 2, targetNode.getBounds().getHeight() / 2);
