 ...
 * @author Gero.Decker
 * 
 *         (1) duplicate the processes, (1.1) replace interaction with send /
 *         receive message events or dummy nodes (1.2) replace XOR data-based
 *         gateways with XOR event-based gateways if necessary (2) reduce
 *         processes, (2.1) remove dummy nodes from processes (2.2) remove empty
 *         subprocesses (2.3) remove unnecessary gateways (using SESE-regions)
 *         (2.4) handle structure following XOR event-based gateways (3) add
 *         message flows
 * 
...
public class IBPMN2BPMNConverter {
    ...
    protected void handleEventBasedGateways(Container cont, ConversionContext c) throws ConversionException {
        boolean more = true; // this strange implementation is needed as the
                             // list is modified
        while (more) {
            more = false;
            for (Node n : cont.getChildNodes())
        ...
        for (Edge e : g.getOutgoingEdges()) {
            if (e instanceof SequenceFlow) {
                ...
                if (n instanceof IntermediateEvent) {
                    ...
                    // (all other possible events are of catching nature (timer,
                    // message))

                } else if (n instanceof EndEvent) {
                    if (n instanceof EndPlainEvent) {
                        // remove node (case (5))
                        // Nota bene: the partner will not be aware of the
                        // conversion end!
                        apply5(e, cont, c);
                        return true;
                    } else {
                        // this cannot happen, as no other end event type is
                    ...
                    if (countSequenceFlows(n.getOutgoingEdges()) == 1) {
                        // event-based gateway followed by XOR merge leads to
                        // duplication
                        // case (1a)
                    ...
                    if (countSequenceFlows(n.getIncomingEdges()) > 1) {
                        // AND-join: not allowed! (BPMN does not support
                        // non-free-choice-ness)
                        // case (3a)
                        throw new ConversionException();
                    } else {
                        apply3b(g, e, cont, c);
