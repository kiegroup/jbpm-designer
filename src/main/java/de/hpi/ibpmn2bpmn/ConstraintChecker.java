 ...
 * @author Gero.Decker
 * 
 *         Constraints to be checked: - no event-based gateways, no OR-gateways,
 *         no Complex Gateways - no multi-instance activities - no attached
 *         intermediate events - no end terminate events - TODO every
 *         (sub-)process has exactly one start and one end event - TODO no pool
 *         sets - TODO every node has at most one incoming and one outgoing edge
 *         (except gateways)
 * 
 */
