...
<xsl:param name="wsdlid" />

<!--
    XForms implementation the generated XForms shall be used with
    Available values: xsltforms
 -->
<xsl:param name="targetImplementation" />

<xsl:param name="writedocuments" select="false()" />
...
<xsl:template match="wsdl:portType/wsdl:operation">
  ...
  <xsl:if test="$targetImplementation = 'xsltforms'">
...
<xsl:template match="wsdl:portType/wsdl:operation" mode="head">
  ...
  <xforms:model>
    ...
    <xforms:toggle case="{$formname}.request" events:event="xforms-ready"/>
  </xforms:model>
</xsl:template>
