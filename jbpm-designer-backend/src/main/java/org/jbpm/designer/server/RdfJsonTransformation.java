 ...
 * org.jbpm.designer.server.RdfJsonTransformation
...
public class RdfJsonTransformation {
    ...
    public static JSONObject toJson(Document rdfDoc,
                                    String requestUrl) {
        ...
        if (root.hasChildNodes()) {
            for (Node node = root.getFirstChild(); node != null; node = node.getNextSibling()) {
                ...
                Node rdfTypeNode = getChild(node,
                                            "rdf:type");
                if (rdfTypeNode != null) {
                    String rdfType = getAttributeValue(rdfTypeNode,
                                                       "rdf:resource");
    ...
    private static void handleCanvas(Node n) {
        try {
            canvas.put("resourceId",
                       getResourceId(n));
            canvas.put("childShapes",
                       new JSONArray());
            handleProperties(n,
                             canvas);
    ...
    private static void handleShape(Node n) {
        try {
            ...
            shape.put("resourceId",
                      resourceId);
            objects.put(resourceId,
                        shape);
            parents.put(resourceId,
                        getParentResourceId(n));
            shape.put("outgoing",
                      new JSONArray());
            shape.put("childShapes",
                      new JSONArray());
            handleProperties(n,
                             shape);
        } catch (JSONException e) {
    ...
    private static void handleProperties(Node n,
                                         JSONObject object) throws JSONException {
        JSONObject properties = new JSONObject();
        object.put("properties",
                   properties);
        if (n.hasChildNodes()) {
            for (Node child = n.getFirstChild(); child != null; child = child.getNextSibling()) {
                if (!isReservedNodeName(child.getNodeName())) {
                    ...
                    if (content == null) {
                        properties.put(child.getNodeName(),
                                       content);
                    } else {
                        try {
...
//        }
    ...
    private static void handleReservedNodeName(Node n,
                                               JSONObject object) throws JSONException {
        ...
        if (n.getNodeName().equals("bounds")) {
            ...
            if (object.has("stencilset")) {
                ...
                object.put("stencilset",
                           stencilset);
            }

            String stencilsetUrl = getAttributeValue(n,
                                                     "rdf:resource");
            ...
            String namespace = getAttributeValue(n,
                                                 "rdf:resource");
    ...
    private static JSONObject getBounds(Node n) throws JSONException {
        ...
        upperLeft.put("x",
                      Double.parseDouble(boundStringArr[0]));
        upperLeft.put("y",
                      Double.parseDouble(boundStringArr[1]));

        JSONObject lowerRight = new JSONObject();
        lowerRight.put("x",
                       Double.parseDouble(boundStringArr[2]));
        lowerRight.put("y",
                       Double.parseDouble(boundStringArr[3]));

        JSONObject bounds = new JSONObject();
        bounds.put("upperLeft",
                   upperLeft);
        bounds.put("lowerRight",
                   lowerRight);
        return bounds;
    }
    ...
    private static JSONArray getDockers(Node n) throws JSONException {
        ...
        String[] dockerPoints = dockersString.replaceAll("#|\\s+",
                                                         " ").trim().split(" ");
    ...
    private static String getParentResourceId(Node n) {
        ...
        String parentResourceId = getResourceId(getAttributeValue(parentNode,
                                                                  "rdf:resource"));
        if (parentResourceId != null) {
            return getResourceId(parentResourceId);
        } else {
            return null;
        }
    }
