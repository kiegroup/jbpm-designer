  ...
  <xsl:choose>
    ...
    <xsl:otherwise>
     <xhtml:div class="form_row">
	  <xsl:choose>
		<xsl:when test="($formtype = 'input') or ($ref = 'return')">
	    ...
	    <xsl:when test="($formtype = 'output') and not($ref = 'return')">
      		<xforms:group ref="tns:{$ref}">
      		  <!-- Generate labels and hints. -->
      		  <xsl:if test="$node">
      		    <xsl:apply-templates select="$node" mode="label"/>
      		  </xsl:if>
...
<xsl:template match="xsd:element" mode="form">
  ...
  <xsl:choose>
    ...
    <xsl:when test="$maxOccurs = 'unbounded' or number($maxOccurs) &gt; 1">
      <xsl:variable name="repeat-id" select="translate($parentref,'():/.''','______')"/>
      <xhtml:div class="form_row">
      ...
      <xsl:if test="$formtype = 'input'">
       <xhtml:div class="form_row">
        ...
        <xforms:trigger ref="delete-trigger-{generate-id()}">
          ...
          <xforms:setindex events:event="DOMActivate" repeat="{$repeat-id}" index="if(index('{$repeat-id}') = count(../{$newref}), index('{$repeat-id}') - 1, index('{$repeat-id}'))"/>
        </xforms:trigger>
       </xhtml:div>
      </xsl:if>
    </xsl:when>
          ...
          <xsl:choose>
            <xsl:when test="$localname = 'base64Binary' or $localname = 'hexBinary'">
              <xforms:trigger ref="{$ref}" appearance="minimal">
                <!-- Generate labels and hints. -->
                <xsl:apply-templates select="$node" mode="label"/>
                <xforms:load events:event="DOMActivate" ref="." show="replace"/>
              </xforms:trigger>
            </xsl:when>
            <xsl:otherwise>
...
<xsl:template match="@name" mode="label label-only">
  <xsl:if test="$debug"><xsl:message select="concat('label-only: ', .)"/></xsl:if>
  <xforms:label><xsl:value-of select="concat(upper-case(substring(.,1,1)),replace(substring(.,2),'[A-Z]',' $0'))"/></xforms:label>
</xsl:template>
...
<xsl:template match="@name" mode="title">
  <xsl:if test="$debug"><xsl:message select="concat('title: ', .)"/></xsl:if>
  <xhtml:title><xsl:value-of select="concat(upper-case(substring(.,1,1)),replace(substring(.,2),'[A-Z]',' $0'))"/></xhtml:title>
</xsl:template>
...
<xsl:template match="@name" mode="heading">
  <xsl:if test="$debug"><xsl:message select="concat('heading: ', .)"/></xsl:if>
  <xhtml:h1><xsl:value-of select="concat(upper-case(substring(.,1,1)),replace(substring(.,2),'[A-Z]',' $0'))"/></xhtml:h1>
</xsl:template>
